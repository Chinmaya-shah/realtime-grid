...
import { User, Edit3, Check, Loader2, Settings, Sun, Moon, Volume2, VolumeX, LogOut, ChevronDown } from "lucide-react";
...
export default function ProfileWidget({
  ...
  return (
    <div className="neumorphic-raised p-6 space-y-5 relative">
      ...
      <div className={`flex items-center justify-between ${!isCollapsed ? "border-b border-outline pb-3" : ""}`}>
        <div
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="flex items-center gap-1.5 cursor-pointer select-none hover:opacity-90 active:scale-98 transition-all"
        >
          ...
          <ChevronDown className={`w-3.5 h-3.5 text-slate-500 transition-transform duration-300 ${!isCollapsed ? "rotate-180" : ""}`} />
        </div>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {settingsOpen && (
            <div className="absolute right-0 mt-2.5 w-48 rounded-2xl border border-outline neumorphic-item p-2 z-50 space-y-1 bg-surface">
              <button
                onClick={() => {
                  onToggleTheme();
                  setSettingsOpen(false);
              ...
              {COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  className={`w-5.5 h-5.5 rounded-lg transition-transform hover:scale-115 active:scale-90 cursor-pointer flex items-center justify-center border relative shadow-sm ${
                    user.color === c ? "ring-2 ring-secondary scale-105 border-transparent" : "border-outline cell-embossed"
                  }`}
                >
                  {user.color === c && <Check className="w-3 text-white stroke-[3px]" />}
                </button>
              ))}
            </div>
          </div>
